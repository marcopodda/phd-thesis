...
A \gls{gcl} is essentially a neural network layer that performs message passing. The term \quotes{convolutional} is used to remark that the local processing performed by the state transition function is a generalization of the convolutional layer for images to graph domains with variable-size neighborhoods. Given a graph $\Graph{g}$ with $n$ nodes, and its node attributes $\Vector{X}_{\Graph{g}} = \Set{\Elem{x}{v} \mid v \in \Nodes_{\Graph{g}}}$, one general formulation of a \gls{gcl} is the following:
\begin{align}
    \label{eq:simple-aggregation}
    \Elem{h}{v}^{\ell} = U \Par{\Elem{h}{v}^{\ell-1}, A\Par{\Set{\,T(\Elem{h}{u}^{\ell-1}) \mid u \in \Neigh(v)}}}, \; \forall v \in \Nodes_{\Graph{g}},
\end{align}
where $\Elem{h}{v}^{\ell} \in \Real^{h_\ell}$ is the hidden state of the node at layer $\ell$, $\,\Elem{h}{v}^{\ell-1} \in \Real^{h_{\ell-1}}$ is the hidden state of the node at the previous layer $\ell-1$, and by convention $\Elem{h}{v}^{0} = \Elem{x}{v}$. Notice that the neighborhood function $\Neigh$ is also implicitly passed as input of the layer, so that the connectivity of each node is known. We can identify three key functions inside a \gls{gcl}:
\begin{itemize}
...
\begin{align}
    ...
    \Elem{h}{v}^{\ell} = \sigmoid\Par{\Matrix{W}^{\Transpose}_{\ell} \sum_{u \in \Neigh(v)} \Matrix{\tilde{L}}_{uv}\Elem{h}{u}^{\ell-1}}, \; \forall v \in \Nodes_G,
\end{align}
where $\Matrix{\tilde{L}}_{uv}$ is the entry of the symmetric normalized graph Laplacian $\Matrix{\tilde{L}}$ related to nodes $u$ and $v$, and:
\begin{align}
    \label{eq:transform}
    T(\Elem{h}{u}^{\ell-1}) &= \Elem{t}{v}^{\ell-1}  = \Matrix{\tilde{L}}_{uv}\Elem{h}{u}^{\ell-1}\\
    \label{eq:aggregate}
    A\Par{\Set{\Elem{t}{u}^{\ell-1} \mid u \in \Neigh(v)}} &= \Elem{n}{v}^{\ell-1} = \sum_{u \in \Neigh(v)}\Elem{t}{u}^{\ell-1}\\
    \label{eq:update}
    U(\Elem{h}{v}^{\ell-1}, \Elem{n}{v}^{\ell-1}) &= \Elem{h}{v}^{\ell} =  \sigmoid\Par{\Matrix{W}^{\Transpose}_{\ell} \Elem{n}{v}^{\ell-1}}.
\end{align}
...
\begin{align*}
    \Elem{h}{v}^{\ell} = \sigmoid\Par{\sum_{c \in \Cal{C}} \Matrix{W}^{\Transpose}_{c,\ell}\sum_{u \in \Neigh_c(v)} \Matrix{\tilde{L}}_{uv}\,\Elem{h}{v}^{\ell-1}},\; \forall v \in \Nodes_{\Graph{g}},
\end{align*}
...
$$A(\Set{\,a^{\ell}_{uv}\,T(\Elem{h}{u}^{\ell-1}) \mid u \in \Neigh(v)}).$$
...
$$w_{vu}^{\ell} = F(\Elem{h}{v}^{\ell}, \Elem{h}{u}^{\ell}),$$
...
As we have seen, the application of $L$ \gls{dgn} layers to a graph $\Graph{g}$ yields $L$ hidden states per node, each one composed with a progressively broad context. In node classification or regression tasks, these are combined by a \emph{hidden state readout} function to obtain a unique hidden state to use as input of the output function, which emits a prediction for every node. Specifically, a hidden state readout function $\Fun{R'}$ computes a \keyword{node representation} (or \keyword{node embeddings}) $\Elem{h}{v}^*$ for each node as follows:
$$\Elem{h}{v}^* = \Op{R}_{v}\Par{\Set{\Elem{h}{v}^{\ell} \mid \ell = 1, \ldots, L}},\;  \forall v \in \Nodes_{\Graph{g}}. $$
...
$$\Elem{o}{v} = \Network(\Elem{h}{v}^*),\; \forall v \in \Nodes_{\Graph{g}},$$
where $\Elem{o}{v} \in \Real^y$ and $\Network$ can be any arbitrarily complex neural network as usual. A visual example of the process for a single node is shown in Figure \ref{fig:node-readout}.
\begin{figure*}[h!]
    \centering
...
\end{figure*}
In graph classification or regression tasks, the node representations computed by a node readout function are aggregated once more by a \emph{graph readout} function, to compute a \keyword{graph representation} (or \keyword{graph embedding}) $\Vector{h}_{\Graph{g}}$, \ie a vector representing the entire graph. Differently from the hidden state readout, the readout function must necessarily be permutation-invariant, since there are no guarantees about the number of graph nodes. Specifically, a graph readout function $\Fun{R}$ computes the embedding of graph $\Graph{g}$ as follows:
$$\Vector{h}_{\Graph{g}} = \Op{R}_{\Graph{g}}\Par{\Set{\Elem{h}{v}^* \mid v \in \Nodes_{\Graph{g}}}}.$$
...
$$\Vector{o} = \Network(\Vector{h}_{\Graph{g}}).$$
\begin{figure*}[h!]
    \centering
    \resizebox{.6\textwidth}{!}{\input{Figures/Chapter4/05-graph-readout}}
    \caption{A graph readout on an example graph for a graph classification/regression task. Here, we assume the node representations $\Elem{h}{v}^{*}$ have already been obtained by a node readout (not shown).}
...
\begin{align*}
    \Matrix{Z}_{\ell-1} &= \Fun{DGN}_e(\Matrix{A}_{\ell-1},\Matrix{H}_{\ell-1}) \in \Real^{n \times h}\\
    \Matrix{S}_{\ell-1} &= \softmax\Par{\Fun{DGN}_p(\Matrix{A}_{\ell-1},\Matrix{H}_{\ell-1})} \in \Real^{n \times k},
\end{align*}
...
\begin{align*}
    \Matrix{H}_{\ell} &= \Matrix{S}_{\ell-1}^{\Transpose}\Matrix{Z}_{\ell-1} \in \Real^{k \times h}\\
    \Matrix{A}_{\ell} &= \Matrix{S}_{\ell-1}^{\Transpose}\Matrix{A}_{\ell-1}\Matrix{S}_{\ell-1} \in \Real^{k \times k}
\end{align*}
where $\Matrix{A}_0 = \Matrix{A}$ and $\Matrix{H}_0 = \Matrix{X}_{\Graph{g}}$. Thus, after applying the DiffPool layer, the size of the graph is reduced progressively from $n$ to $k$ nodes.
...
$$\argmin_{\Param} \Loss(\Param, (\Vector{x}_{\Graph{g}}, y)) + \lambda \sum_{\ell=1}^L \Psi(\Matrix{H}^{\ell}_{\Graph{g}}),$$
...
\begin{align}
    \Psi(\Matrix{H}^{\ell}_{\Graph{g}}) = \sum_{u,v} \Norm{\Elem{h}{v}^{\ell} - \Elem{h}{u}^{\ell}}_2,
\end{align}
...
$$\Elem{h}{v}^{\ell} = \Fun{MLP} \Par{(1 + \epsilon^{\ell})\,\Elem{h}{v}^{\ell-1} + \sum_{u \in \Neigh(v)} \Elem{h}{u}^{\ell-1}},$$
...
$$\Elem{h}{v}^{\ell} = \sigma\Par{\Matrix{W}^{\Transpose}_{\ell}\Elem{h}{u}^{\ell-1} + \frac{1}{|\Neigh(v)|} \Matrix{U}^{\Transpose}_{\ell} \sum_{u \in \Neigh(v)} \Elem{h}{u}^{\ell-1}}.$$
...
$$\Elem{h}{v}^{\ell} = \sigma\Par{ \frac{1}{|\Neigh(v)|}\sum_{u \in \Neigh(v)} \Fun{MLP}(\Elem{e}{u,v})^{\Transpose} \Elem{h}{u}^{\ell-1}},$$
...
$$\Elem{h}{v}^{\ell} = \Fun{tanh} \Par{\Matrix{W}_{\ell}^{\Transpose} \sum_{u \in \Neigh(v)} \Matrix{D}^{-1}_{vu}\Elem{h}{u}^{\ell-1}},$$
...
We compare the proposed architectures with two structure-agnostic baselines, one for molecular graphs, and one for social graphs. For molecular graphs, with the exception of graphs in the ENZYMES dataset, we use an \gls{mlp}-based model \citep{ralaivola2005graphkernels}, applied as follows to a generic graph $\Graph{g}$:
$$\Vector{h}_{\Graph{g}} = \Fun{MLP}\Par{\sum_{v \in \Nodes_{\Graph{g}}} \Elem{x}{v}}.$$
...
For the social graphs, and for the graphs in the ENZYMES dataset (which uses 18 additional node features with respect to the other molecular datasets), we use the following architecture, applied to a generic graph $\Graph{g}$ as follows:
$$\Vector{h}_{\Graph{g}} = \Fun{MLP}_2\Par{\sum_{v \in \Nodes_{\Graph{g}}} \Fun{MLP}_1(\Elem{x}{v})},$$
